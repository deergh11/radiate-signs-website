import { getProjectCardBySlug } from '@/data/project-index'
import type { CaseStudyProject, ProjectCard } from '@/data/project-types'

function requireProjectCard(slug: string): ProjectCard {
  const project = getProjectCardBySlug(slug)

  if (!project) {
    throw new Error(`Missing project card for slug "${slug}"`)
  }
